
This serves only as a proof that I invented this algorithm, in case anyone claims that he came up with a number vectorizer/unifier.
The code for it is in **NumLab.js**, from which stems the crypting mechanism found in **uncrunchable.js**. It converts a Uint32 array into a number of arbitrary-length, performs encryption, then converts the number back to a _UInt32_ array
NumLab was first published to GitHub on _July 24th 2026_.

NumLab can be tested [on this webpage](http://1zz.online/NumLab/index.html)

Read discussion on forum: https://masm32.com/board/index.php?topic=13322.0


You may not reproduce or recompile this code, but you are allowed to use the crypting mechanism from an external source. Please see 
